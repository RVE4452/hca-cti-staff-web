  ...
  <div class="modal" v-bind:class="isPanelOpen ? 'sideNavPanel' : 'sideNavPanelClose'">
    <div style="text-align: center">
      <neu-header class="fs-22 fw-bold modal-header relative" id="modalTitle">
        <f5
          ><span>{{ requestTypeArray[sharedRequest.type - 1] }} </span></f5
        >
        <neu-icon
          @click="close"
          data-test="modal-close"
          class="btn-close f1 absolute pointer btnSidePanelClose m-3 neu-icon neu-icon--large hydrated"
          >close</neu-icon
        >
      </neu-header>
          ...
          <div>
            <h5>
              ...
              <span>
                {{
                  sharedRequest &&
                  sharedRequest.calSelectedDates &&
                  sharedRequest.calSelectedDates.startDate
                    ? formatDate(sharedRequest.calSelectedDates.startDate)
    ...
    <div>
      ...
      <div v-if="tabCurrent === 'Trade'">
        <TradeShift
          ...
          @showSuccessMsgPopUp="showSuccessModal"
          @closeSharedModal="close"
        />
      </div>
...
import { NeuHeader, NeuRow, NeuCol } from "@neutron/vue";
...
export default class SharedModal extends Vue.with(Props) {
  ...
  onMatchChanged() {
    if (this.isPanelOpen) {
      document
        .getElementsByClassName("neu-padding--bottom-100")[0]
        .classList.add("postion-relative", "overflow-hidden");
    } else {
      document
        .getElementsByClassName("neu-padding--bottom-100")[0]
        .classList.remove("postion-relative", "overflow-hidden");
    }
  }
  ...
  previousDay() {
    ...
    if (
      this.sharedRequest &&
      this.sharedRequest.calSelectedDates &&
      this.sharedRequest.calSelectedDates.startDate
    ) {
  ...
  showDateNavigation() {
    ...
    if (
      ...
      startDate = moment(this.sharedRequest.calSelectedDates.startDate).format(
        "MMMM DD, yyyy"
      );
      endDate = moment(this.sharedRequest.calSelectedDates.endDate)
        .add(-1, "days")
        .format("MMMM DD, yyyy");
      if (startDate === endDate) ret = true;
      else ret = false;
    }
    return ret;
  }
