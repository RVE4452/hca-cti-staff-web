    ...
    <div>
        <neu-container class="mw-100">
            <div class="schedulerMainDiv" v-bind:class="{ 'panel-open': sharedToggle }">
                <neu-row v-if="viewFlag == 'CalView'">
                        ...
                        <h5>
                            <p left="true">
                                <small @click.prevent="todayDate()">
                                    ...
                                    <a style="position: relative; top: -7px;" class="neu-link" href="#">TODAY</a>
                                </small>
                            </p>
                        </h5>
                    </neu-col>
                    <neu-col md="6">
                        <div>
                            <div style="text-align: center">
                                <h5>
                                    <neu-icon @click="beforeMonthNavigate(false)" style="position: relative; top: 4px"
                                        class="m-3 neu-icon pointer neu-icon--large hydrated">chevron_left</neu-icon>
                                    <span>
                                        {{ currentDate }}
                                    </span>
                                    <neu-icon @click="beforeMonthNavigate(true)" style="position: relative; top: 4px"
                                        class="m-3 neu-icon pointer neu-icon--large hydrated">chevron_right</neu-icon>
                                </h5>
                            </div>
                        </div>
                    </neu-col>
                    <neu-col md="2">
                        <h5>
                            <p style="margin-top: 1em; margin-left: 1em;">
                                ...
                                <span
                                    v-if="scheduleStatus.toUpperCase() == 'BEFORE PLAN SHEET' || scheduleStatus.toUpperCase() == 'FUTURE'">
                                    FUTURE SCHEDULE
                                </span>
                            </p>
                        </h5>
                    ...
                    <neu-col md="1" style="margin-left:20px;">
                        <neu-icon @click="toggleFilterPanel()"
                            class="m-3 neu-icon pointer neu-icon--large hydrated">filter_list</neu-icon>
                    </neu-col>
                </neu-row>
                <neu-row>
                    <div class="container calContainer" v-if="viewFlag == 'CalView'">
                        <div v-if="!isLoading">
                            ...
                            <neu-row>
                                <neu-col md="12">
                                    <FullCalendar :options="calendarOptions" ref="fullCalendarCurrentMonth">
                                    </FullCalendar>
                                </neu-col>
                                <div class="col-sm-12 neu-margin--top-20 neu-text--caption neu-text--align-center mTop0">
                                    <span class="neu-text--bold">Important Note </span>: Shift assignments display on the
                                    date of the shift's actual start time.
                                </div>
                            </neu-row>
                            <!-- END CALENDAR -->
                            <!-- MODAL -->
            ...
            <neu-row style="display: block;">
                <CalendarFilterPanel ref="calendarfilter" @close-event="toggleFilterPanel" @sched-event="onCheckSchedEvents"
                    @check-pend-event="onCheckPendRequestsOption" @check-depart-need-event="onCheckDepartNeedsOption"
                    @check-trade-shift-event="onCheckTradeShiftsOption"
                    @check-unavailability-event="onCheckUnavailabilityOption" @check-all="onCheckAllOption"
                ...
                if (res === '' || res.status === 'Accepted') {
                    ...
                    this.$store
                        ...
                        .then(() => {
                            ...
                            useAppInsights().trackEvent({
                                name: appinsightTitle, properties:
                                    JSON.parse(JSON.stringify(this.appInsightEventData))
                            });
                        })
                        .catch((err: any) => {
